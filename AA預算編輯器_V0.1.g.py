...
class BudgetEditor(tk.Tk):
    ...
    def _build_quantity_table(self):
        ...
        for col in range(QTY_COLUMNS):
            ...
            self.quantity_parent.columnconfigure(col + 1, weight=1, minsize=widths[col] * 10)
        ...
        for row_index, row_data in enumerate(self.quantity_data):
            ...
            for col in range(QTY_COLUMNS):
                ...
                is_readonly = col != QTY_EXPR_INDEX
                entry = tk.Entry(
    ...
    def _update_cell_data(self, row_index, column_index):
        if self._is_rendering:
            return
        self.rows_data[row_index][column_index] = self.cell_vars[row_index][column_index].get()
