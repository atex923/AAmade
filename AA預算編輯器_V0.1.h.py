...
class BudgetEditor(tk.Tk):
    def __init__(self):
        ...
        self.quantity_col_widths = [100, 160, 220, 180, 100, 220, 140]
    ...
    def _build_layout(self):
        ...
        ttk.Label(tools, textvariable=self.top_message_var, foreground="#666666").grid(row=0, column=12, sticky="w")
        ...
        ttk.Button(tools, text="回傳計算結果", command=self.return_quantity_results).grid(
    ...
    def _build_quantity_table(self):
        ...
        self.quantity_vars = []
        self.quantity_entries = []
        headers = ["項次", "工項號碼", "工項名稱", "規格", "單位", "計算式", "計算結果"]
        ...
        for row_index, row_data in enumerate(self.quantity_data):
            ...
            for col in range(QTY_COLUMNS):
                ...
                is_readonly = col not in (*QTY_SYNC_COLUMNS, QTY_EXPR_INDEX)
                entry = tk.Entry(
    ...
    def _update_cell_data(self, row_index, column_index):
        ...
        if column_index in QTY_SYNC_COLUMNS and row_index < len(self.quantity_data):
            self.quantity_data[row_index][column_index] = value
            if row_index < len(self.quantity_vars) and column_index < len(self.quantity_vars[row_index]):
                target_var = self.quantity_vars[row_index][column_index]
    ...
    def _sync_quantity_source(self):
        ...
        for row in self.quantity_data:
            ...
            old_results.append(row[QTY_RESULT_INDEX] if len(row) > QTY_RESULT_INDEX else "")

        quantity_rows = []
        for row_index, source_row in enumerate(self.rows_data):
            row = list(source_row[:QTY_EXPR_INDEX])
            expression = old_expressions[row_index] if row_index < len(old_expressions) else source_row[QTY_EXPR_INDEX]
            result = old_results[row_index] if row_index < len(old_results) else ""
    ...
    def _update_quantity_synced_cell(self, row_index, column_index):
        ...
        if row_index < len(self.cell_vars) and column_index < len(self.cell_vars[row_index]):
            target_var = self.cell_vars[row_index][column_index]
            if target_var.get() != value:
                target_var.set(value)
    ...
    def return_quantity_results(self):
        self._sync_model_from_vars()
        self._sync_quantity_from_vars()
        self._record_history()
        code_to_row = {}
        for row_index, row in enumerate(self.rows_data):
            ...
            if code:
                code_to_row[code] = row_index

        returned_count = 0
        missing_codes = []
        for quantity_row in self.quantity_data:
            code = self._normalize_code(quantity_row[CODE_COLUMN_INDEX])
            result = quantity_row[QTY_RESULT_INDEX].strip()
            if not code or not result or result == "錯誤":
                continue
            ...
            self.rows_data[target_row][5] = result
            if target_row < len(self.cell_vars):
                target_var = self.cell_vars[target_row][5]
                if target_var.get() != result:
                    target_var.set(result)
            returned_count += 1

        if missing_codes:
            messagebox.showwarning("回傳計算結果", "找不到對應工項號碼：\n" + "\n".join(missing_codes[:20]))

        self._build_table()
        self.top_message_var.set(f"已回傳 {returned_count} 筆計算結果到第1分頁 F 欄。")
