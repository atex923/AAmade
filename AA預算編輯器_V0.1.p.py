...
class BudgetEditor(tk.Tk):
    ...
    def _build_layout(self):
        ...
        top_area = ttk.Frame(self, padding=(10, 8, 10, 6))
        ...
        ttk.Label(top_area, text="工程位置：").grid(row=1, column=0, sticky="w", padx=(0, 6), pady=(4, 0))
        ...
        ttk.Label(top_area, text="工程內容：").grid(row=2, column=0, sticky="nw", padx=(0, 6), pady=(4, 0))
        self.project_content_text = tk.Text(top_area, height=2, wrap="word", relief="solid", borderwidth=1)
        self.project_content_text.grid(row=2, column=1, columnspan=5, sticky="ew", pady=(4, 0))

        tools = ttk.Frame(top_area)
        tools.grid(row=3, column=0, columnspan=6, sticky="ew", pady=(6, 0))
        tools.columnconfigure(0, weight=0)
        tools.columnconfigure(1, weight=1)
        tools.columnconfigure(2, weight=1)

        edit_tools = ttk.LabelFrame(tools, text="行位與編輯", padding=(8, 4))
        edit_tools.grid(row=0, column=0, sticky="nsew", padx=(0, 8))

        view_tools = ttk.LabelFrame(tools, text="顯示與搜尋", padding=(8, 4))
        view_tools.grid(row=0, column=1, sticky="nsew", padx=(0, 8))

        action_tools = ttk.LabelFrame(tools, text="記錄與分頁執行", padding=(8, 4))
        action_tools.grid(row=0, column=2, sticky="nsew")

        ttk.Label(edit_tools, text="插入行位數量").grid(row=0, column=0, sticky="w", padx=(0, 6))
        ...
        insert_button = ttk.Button(edit_tools, text="插入行位", command=self.insert_rows_at_cursor)
        insert_button.grid(row=0, column=2, sticky="w", padx=(0, 8))

        delete_button = ttk.Button(edit_tools, text="刪除行位", command=self.delete_row_at_cursor)
        delete_button.grid(row=0, column=3, sticky="w", padx=(0, 8))

        ttk.Button(edit_tools, text="復原刪除", command=self.undo_delete_row).grid(row=1, column=0, sticky="w", padx=(0, 8), pady=(4, 0))
        ttk.Button(edit_tools, text="回復修改", command=self.undo_last_change).grid(row=1, column=1, sticky="w", padx=(0, 8), pady=(4, 0))
        ttk.Button(edit_tools, text="重做修改", command=self.redo_last_change).grid(row=1, column=2, sticky="w", padx=(0, 8), pady=(4, 0))

        ttk.Label(view_tools, text="隱藏階層").grid(row=0, column=0, sticky="w", padx=(0, 6))
        ...
        hide_level.grid(row=0, column=1, sticky="w", padx=(0, 8))
        ttk.Button(view_tools, text="套用", command=self.hide_level_rows).grid(row=0, column=2, sticky="w", padx=(0, 8))

        ttk.Label(view_tools, text="指定工項").grid(row=0, column=3, sticky="w", padx=(0, 6))
        self.show_code_var = tk.StringVar()
        show_code = ttk.Entry(view_tools, textvariable=self.show_code_var, width=13)
        show_code.grid(row=0, column=4, sticky="w", padx=(0, 8))
        ttk.Button(view_tools, text="顯示以下階層", command=self.show_only_code_tree).grid(row=0, column=5, sticky="w", padx=(0, 8))
        ttk.Button(view_tools, text="恢復顯示全部", command=self.show_all_rows).grid(row=0, column=6, sticky="w", padx=(0, 8))

        self.top_message_var = tk.StringVar(value="插入位置會從目前游標所在列的下一行開始插入。")

        ttk.Label(view_tools, text="跳到工項").grid(row=1, column=0, sticky="w", padx=(0, 6), pady=(4, 0))
        self.jump_code_var = tk.StringVar()
        ttk.Entry(view_tools, textvariable=self.jump_code_var, width=13).grid(row=1, column=1, sticky="w", padx=(0, 8), pady=(4, 0))
        ttk.Button(view_tools, text="跳到", command=self.jump_to_code).grid(row=1, column=2, sticky="w", padx=(0, 8), pady=(4, 0))

        ttk.Label(view_tools, text="搜尋關鍵字").grid(row=1, column=3, sticky="w", padx=(0, 6), pady=(4, 0))
        self.search_keyword_var = tk.StringVar()
        ttk.Entry(view_tools, textvariable=self.search_keyword_var, width=16).grid(row=1, column=4, sticky="w", padx=(0, 8), pady=(4, 0))
        ttk.Button(view_tools, text="搜尋", command=self.search_keyword).grid(row=1, column=5, sticky="w", padx=(0, 8), pady=(4, 0))

        ttk.Label(action_tools, text="檔名").grid(row=0, column=0, sticky="w", padx=(0, 6))
        self.save_filename_var = tk.StringVar()
        ttk.Entry(action_tools, textvariable=self.save_filename_var, width=18).grid(row=0, column=1, sticky="w", padx=(0, 8))
        ttk.Button(action_tools, text="儲存記錄", command=self.save_record).grid(row=0, column=2, sticky="w", padx=(0, 8))
        ttk.Button(action_tools, text="開啟記錄", command=self.open_record).grid(row=0, column=3, sticky="w", padx=(0, 8))
        ttk.Button(action_tools, text="輸出第2分頁", command=self.export_cover_content_excel).grid(
            row=1, column=0, sticky="w", padx=(0, 8), pady=(4, 0)
        )
        ttk.Button(action_tools, text="執行第3分頁計算", command=self.calculate_quantity_page).grid(
            row=1, column=1, sticky="w", padx=(0, 8), pady=(4, 0)
        )
        ttk.Button(action_tools, text="執行整合單價分析", command=self.integrate_unit_analysis).grid(
            row=1, column=2, columnspan=2, sticky="w", padx=(0, 8), pady=(4, 0)
        )

        lower_area = ttk.Frame(self, padding=(10, 2, 10, 8))
        ...
        self.main_notebook.add(table_shell, text="預算總表")
        ...
        analysis_shell = ttk.Frame(self.main_notebook)
        self.main_notebook.add(analysis_shell, text="第4頁 單價分析")
        analysis_shell.columnconfigure(0, weight=1)
        analysis_shell.rowconfigure(0, weight=1)
        self._build_unit_analysis_page(analysis_shell)

        bottom_bar = ttk.Frame(self, padding=(10, 0, 10, 6))
        ...
        status = ttk.Label(bottom_bar, textvariable=self.status_var, anchor="w")
        status.grid(row=0, column=0, sticky="ew", padx=(0, 10))
        motto = ttk.Label(bottom_bar, text="Η ανάγκη είναι η μητέρα της εφεύρεσης.", anchor="center")
        motto.grid(row=0, column=1, sticky="ew", padx=10)
        message = ttk.Label(bottom_bar, textvariable=self.top_message_var, anchor="e", foreground="#666666")
        message.grid(row=0, column=2, sticky="ew", padx=(10, 0))
    ...
    def _build_cover_content_page(self, parent):
        ...
        if DEFAULT_COVER_TEMPLATE.exists():
            if not self._build_cover_template_grid(content):
    ...
    def _build_cover_template_grid(self, parent):
        try:
            ...
            self.top_message_var.set("缺少 openpyxl 套件，第2分頁改用備用表格顯示。")
            return False

        try:
            workbook = load_workbook(DEFAULT_COVER_TEMPLATE, data_only=False)
        except (OSError, PermissionError) as error:
            self.top_message_var.set(f"無法讀取第2分頁範本，已改用備用表格：{error}")
            return False
    ...
    def _build_unit_analysis_page(self, parent):
        canvas = tk.Canvas(parent, highlightthickness=0)
        ...
        self.analysis_parent = ttk.Frame(canvas)
        window = canvas.create_window((0, 0), window=self.analysis_parent, anchor="nw")
        self.analysis_parent.bind("<Configure>", lambda _event: canvas.configure(scrollregion=canvas.bbox("all")))
        canvas.bind("<Configure>", lambda event: canvas.itemconfigure(window, width=max(event.width, 1120)))
        self._build_unit_analysis_table()

    def _build_unit_analysis_table(self):
        if self.analysis_parent is None:
            return

        for child in self.analysis_parent.winfo_children():
            child.destroy()
        ...
        display_rows = [list(row) for row in self.analysis_data]
        while len(display_rows) < MIN_DATA_ROWS:
            display_rows.append(["" for _column in range(ANALYSIS_COLUMNS)])

        corner = self._label(self.analysis_parent, "", "header")
        corner.grid(row=0, column=0, sticky="nsew")
        for col in range(ANALYSIS_COLUMNS):
            label = self._label(self.analysis_parent, self._excel_column_name(col + 1), "header")
            label.grid(row=0, column=col + 1, sticky="nsew")
            self.analysis_parent.columnconfigure(col + 1, weight=1, minsize=widths[col] * 10)

        header_row_label = self._label(self.analysis_parent, "1", "row_header")
        header_row_label.grid(row=1, column=0, sticky="nsew")
        for col, text in enumerate(headers):
            label = self._label(self.analysis_parent, text, "field_header")
            label.grid(row=1, column=col + 1, sticky="nsew")

        for row_index, row_data in enumerate(display_rows):
            visual_row = row_index + 2
            row_label = self._label(self.analysis_parent, str(visual_row), "row_header")
            row_label.grid(row=visual_row, column=0, sticky="nsew")

            row_vars = []
            row_entries = []
            self.analysis_vars.append(row_vars)
            self.analysis_entries.append(row_entries)
            for col in range(ANALYSIS_COLUMNS):
                ...
                entry = tk.Entry(
                    ...
                    justify="center" if col == ITEM_COLUMN_INDEX else "left",
                    state="readonly",
                )
    ...
    def _sync_quantity_source(self):
        ...
        for row_index, source_row in enumerate(self.rows_data):
            ...
            result_source_column = self._quantity_to_source_column(QTY_RESULT_INDEX)
            result = (
                source_row[result_source_column]
                if result_source_column is not None and result_source_column < len(source_row)
                else ""
            )
    ...
    def _snapshot_state(self):
        return {
            ...
            "analysis_rows": [list(row) for row in self.analysis_data],
            "project": {
    ...
    def _restore_snapshot(self, snapshot):
        ...
        try:
            ...
            self.analysis_data = [list(row) for row in snapshot.get("analysis_rows", [])]
            self._build_table()
            self._build_unit_analysis_table()
        finally:
            self._history_suspended = False
    ...
    def integrate_unit_analysis(self):
        active_row = self.focused_cell[0] if self.focused_cell else 0
        self.validate_all(active_row)
        self._sync_budget_table_for_analysis()
        self._record_history()
        leaf_rows = self._collect_01_leaf_rows()
        self.analysis_data = []
        self.analysis_data = self._renumber_analysis_rows(leaf_rows)
        self._build_unit_analysis_table()
        self.main_notebook.select(3)

        if leaf_rows:
            self.top_message_var.set(f"已整合 01 工項以下最後階層資料，共 {len(leaf_rows)} 筆。")
        else:
            self.top_message_var.set("01 工項以下沒有可整合的最後階層資料。")

    def _collect_01_leaf_rows(self):
        code_rows = []
        for row in self.rows_data:
            code = self._normalize_code(row[CODE_COLUMN_INDEX])
            if code:
                code_rows.append((code, row))

        codes = [code for code, _row in code_rows]
        leaf_rows = []
        for code, row in code_rows:
            if not code.startswith("01"):
                continue
            if self._has_child_code(code, codes):
                continue
            leaf_rows.append(list(row))
        return leaf_rows

    def _has_child_code(self, code, codes):
        return any(other_code != code and other_code.startswith(code) for other_code in codes)

    def _renumber_analysis_rows(self, rows):
        renumbered_rows = []
        for index, row in enumerate(rows, start=1):
            values = list(row[:ANALYSIS_COLUMNS])
            values.extend([""] * (ANALYSIS_COLUMNS - len(values)))
            values[ITEM_COLUMN_INDEX] = str(index)
            renumbered_rows.append(values[:ANALYSIS_COLUMNS])
        return renumbered_rows

    def _sync_budget_table_for_analysis(self):
        if self.cell_vars:
            self._sync_model_from_vars()
        self._renumber_items()
    ...
    def export_cover_content_excel(self):
        ...
        if DEFAULT_COVER_TEMPLATE.exists():
            try:
                ...
                self.top_message_var.set(f"無法讀取第2分頁範本，改用備用格式輸出：{error}")
            else:
    ...
    def _record_payload(self):
        ...
        return {
            ...
            "analysis_rows": self.analysis_data,
        }
    ...
    def _load_record_payload(self, payload):
        ...
        for row in analysis_rows:
            values = list(row[:ANALYSIS_COLUMNS]) if isinstance(row, list) else []
            values.extend([""] * (ANALYSIS_COLUMNS - len(values)))
            clean_analysis_rows.append(values[:ANALYSIS_COLUMNS])
        self.analysis_data = clean_analysis_rows
        self.show_all_rows()
        self._build_table()
        self._build_unit_analysis_table()
        ...
        if self.main_notebook is not None:
            try:
                ...
                if current_index == 1:
                    ...
                    active_canvas = self.analysis_canvas
            except tk.TclError:
                active_canvas = self.table_canvas
